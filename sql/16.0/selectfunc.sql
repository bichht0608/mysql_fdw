...
-- ===============================================================================
-- test format() of PostgreSQL
-- ===============================================================================
-- normal format
--Testcase 4361:
EXPLAIN VERBOSE 
SELECT format('|%s:%s:%s:%s:%s:%s:%s:%s|', id, tag1, value1, value2, value3, value4, str2, str2) FROM s3;
--Testcase 4362:
SELECT format('|%s:%s:%s:%s:%s:%s:%s:%s|', id, tag1, value1, value2, value3, value4, str2, str2) FROM s3;

--Testcase 4363:
EXPLAIN VERBOSE 
SELECT format('|%-s:%-s:%-s:%-s|', value3, value4, str1, str2) FROM s3;
--Testcase 4364:
SELECT format('|%-s:%-s:%-s:%-s|', value3, value4, str1, str2) FROM s3;

--Testcase 4365:
EXPLAIN VERBOSE 
SELECT format('|%2$s:3$s:$s:4$s|', value3, value4, str1, str2) FROM s3;
--Testcase 4366:
SELECT format('|%2$s:3$s:$s:4$s|', value3, value4, str1, str2) FROM s3;

--Testcase 4367:
EXPLAIN VERBOSE 
SELECT format('|%10s:%9s:%6s:%7s|', value3, value4, str1, str2) FROM s3;
--Testcase 4368:
SELECT format('|%10s:%9s:%6s:%7s|', value3, value4, str1, str2) FROM s3;

--Testcase 4369:
EXPLAIN VERBOSE 
SELECT format('|%-*s:%3$-9s:%*s:%6s|', 10, value3, value4, 5, str1, str2) FROM s3;
--Testcase 4370:
SELECT format('|%-*s:%3$-9s:%*s:%6s|', 10, value3, value4, 5, str1, str2) FROM s3;
...
--Testcase 4371:
EXPLAIN VERBOSE 
SELECT format('|%2s:%3s:%4s:%5s|', id, tag1, value1, value2) FROM s3;
--Testcase 4372:
SELECT format('|%2s:%3s:%4s:%5s|', id, tag1, value1, value2) FROM s3;

--Testcase 4373:
EXPLAIN VERBOSE 
SELECT value1, format('|%*s:%9s:%5s:%6s|', 10, value3, value4, str1, str2) FROM s3 WHERE to_hex(value2) = '64';
--Testcase 4374:
SELECT value1, format('|%*s:%9s:%5s:%6s|', 10, value3, value4, str1, str2) FROM s3 WHERE to_hex(value2) = '64';

-- format with specified flag and width
--Testcase 4375:
EXPLAIN VERBOSE 
SELECT format('|%-*s:%9s:%*s:%6s|', 10, value3, value4, 5, str1, str2) FROM s3;
--Testcase 4376:
SELECT format('|%-*s:%9s:%*s:%6s|', 10, value3, value4, 5, str1, str2) FROM s3;

-- format with left alignment and different type
--Testcase 4377:
EXPLAIN VERBOSE 
SELECT value1, format('|%-20s-%30I|', id, tag1) FROM s3 ORDER BY format('|%-20s-%30I|', id, tag1);
--Testcase 4378:
SELECT value1, format('|%-20s-%30I|', id, tag1) FROM s3 ORDER BY format('|%-20s-%30I|', id, tag1);

--Testcase 4379:
EXPLAIN VERBOSE 
SELECT value1, format('|%-20s-%30I|', id, value1) FROM s3 ORDER BY format('|%-20s-%30I|', id, value1);
--Testcase 4380:
SELECT value1, format('|%-20s-%30I|', id, value1) FROM s3 ORDER BY format('|%-20s-%30I|', id, value1);

--Testcase 4381:
EXPLAIN VERBOSE 
SELECT format('|%20s-%-30L|', id, tag1) FROM s3;
--Testcase 4382:
SELECT format('|%20s-%-30L|', id, tag1) FROM s3;

-- format with customized positions
--Testcase 4383:
EXPLAIN VERBOSE 
SELECT format('|%3$s:%2$s:%4$s:%1$s|', id, tag1, value1, value2) FROM s3;
--Testcase 4384:
SELECT format('|%3$s:%2$s:%4$s:%1$s|', id, tag1, value1, value2) FROM s3;

-- format with mixed attributes
--Testcase 4385:
EXPLAIN VERBOSE 
SELECT count(value1), format('|%6$*3$s:%4$s:%1$*5$s:%s|', id, tag1, -10, value1, 20, value2) FROM s3 GROUP BY format('|%6$*3$s:%4$s:%1$*5$s:%s|', id, tag1, -10, value1, 20, value2);
--Testcase 4386:
SELECT count(value1), format('|%6$*3$s:%4$s:%1$*5$s:%s|', id, tag1, -10, value1, 20, value2) FROM s3 GROUP BY format('|%6$*3$s:%4$s:%1$*5$s:%s|', id, tag1, -10, value1, 20, value2);

--Testcase 4387:
EXPLAIN VERBOSE
SELECT count(value1), format('|%20s-%-30L|', id, tag1) FROM s3 GROUP BY format('|%20s-%-30L|', id, tag1), value1 HAVING format('|%20s-%-30L|', id, tag1) IS NOT NULL;
--Testcase 4388:
SELECT count(value1), format('|%20s-%-30L|', id, tag1) FROM s3 GROUP BY format('|%20s-%-30L|', id, tag1), value1 HAVING format('|%20s-%-30L|', id, tag1) IS NOT NULL;
...
-- ===============================================================================
-- test regexp_like() of PostgreSQL
-- ===============================================================================
--Testcase 4401:
INSERT INTO s3 VALUES (6, 'a', 0.1, 100, -0.1, -100, '---XYZ---', 'a.d');
--Testcase 4402:
INSERT INTO s3 VALUES (7, 'a', 0.1, 100, -0.1, -100, '---XYZ---', 'a . c');
--Testcase 4403:
EXPLAIN VERBOSE
SELECT regexp_like('   XYZ   ', str2) FROM s3;
--Testcase 4404:
SELECT regexp_like('   XYZ   ', str2) FROM s3;
--Testcase 4405:
EXPLAIN VERBOSE
SELECT regexp_like('a'||CHR(10)||'d', str2, 'n') FROM s3 WHERE id = 6;
--Testcase 4406:
SELECT regexp_like('a'||CHR(10)||'d', str2, 'n') FROM s3 WHERE id = 6;
--Testcase 4407:
EXPLAIN VERBOSE
SELECT regexp_like('a'||CHR(10)||'d', str2, 's') FROM s3 WHERE id = 6;
--Testcase 4408:
SELECT regexp_like('a'||CHR(10)||'d', str2, 's') FROM s3 WHERE id = 6;
--Testcase 4409:
EXPLAIN VERBOSE
SELECT regexp_like('abc', str2, 'x') FROM s3 WHERE id = 7;
--Testcase 4410:
SELECT regexp_like('abc', str2, 'x') FROM s3 WHERE id = 7;
--Testcase 4411:
EXPLAIN VERBOSE
SELECT regexp_like('   XYZ   ', str2, 'g') FROM s3;
--Testcase 4412:
SELECT regexp_like('   XYZ   ', str2, 'g') FROM s3;  -- error
--Testcase 4413:
DELETE FROM s3 WHERE id = 6 OR id = 7;

-- ===============================================================================
-- test mysql_regexp_like() for MYSQL
-- ===============================================================================
...
-- select date_add and ALIAS
--Testcase 2313:
SELECT date_add(date_sub(c3, '1 12:59:10')::timestamp,  '1-2'::interval) as date_add1 FROM time_tbl;

-- DATE_ADD() of Postgres
-- Do not push down built-in Postgres function.
-- select date_add (built-in function, explain)
--Testcase 4389:
EXPLAIN VERBOSE
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl;

-- select date_add (built-in function, result)
--Testcase 4390:
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl;

-- select date_add (built-in function, not pushdown constraints, explain)
--Testcase 4391:
EXPLAIN VERBOSE
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE to_hex(id) = '1';

-- select date_add (built-in function, not pushdown constraints, result)
--Testcase 4392:
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE to_hex(id) = '1';

-- select date_add (built-in function, pushdown constraints, explain)
--Testcase 4393:
EXPLAIN VERBOSE
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE id != 1;

-- select date_add (built-in function, pushdown constraints, result)
--Testcase 4394:
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE id != 1;

-- select date_add (built-in function, date_add in constraints, explain)
--Testcase 4395:
EXPLAIN VERBOSE
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE date_add(c2, '1 12:59:10'::interval) != '2000-01-01';

-- select date_add (built-in function, date_add in constraints, result)
--Testcase 4396:
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE date_add(c2, '1 12:59:10'::interval) != '2000-01-01';
...
--Testcase 4397:
EXPLAIN VERBOSE
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE date_add('2021-01-02', '1-2'::interval) > '2000-01-01';

-- select date_add (built-in function, date_add in constraints, result)
--Testcase 4398:
SELECT date_add(c2, '1 12:59:10'::interval), date_add('2021-01-02', '1-2'::interval), date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), date_add('2021-01-02', '1-2'::interval) FROM time_tbl WHERE date_add('2021-01-02', '1-2'::interval) > '2000-01-01';

-- select date_add with agg (not pushdown, explain)
--Testcase 4399:
EXPLAIN VERBOSE
SELECT max(c3), date_add(max(c2), '1-2'::interval) FROM time_tbl;

-- select date_add as nest function with agg (not pushdown, result)
--Testcase 4400:
SELECT max(c3), date_add(max(c2), '1-2'::interval) FROM time_tbl;
...
--Testcase 4401:
EXPLAIN VERBOSE
SELECT date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval), pi(), 4.1 FROM time_tbl;
...
--Testcase 4403:
EXPLAIN VERBOSE
SELECT id, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl order by date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval);

-- select date_add with order by (result)
--Testcase 4404:
SELECT id, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl order by date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval);

-- select date_add with order by index (explain)
--Testcase 4405:
EXPLAIN VERBOSE
SELECT id, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl order by 2,1;
...
--Testcase 4407:
EXPLAIN VERBOSE
SELECT id, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl order by 1,2;
...
--Testcase 4409:
EXPLAIN VERBOSE
SELECT max(c3), date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl group by date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval);
...
-- select date_add with group by index (result)
--Testcase 4411:
SELECT id, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl group by 2,1;

-- select date_add with group by index (result)
--Testcase 4412:
SELECT id, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) FROM time_tbl group by 1,2;

-- select date_add with group by having (explain)
--Testcase 4413:
EXPLAIN VERBOSE
SELECT max(c3), date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c2 FROM time_tbl group by date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c3,c2 HAVING date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) > '2000-01-01';

-- select date_add with group by having (result)
--Testcase 4414:
SELECT max(c3), date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c2 FROM time_tbl group by date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c3,c2 HAVING date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) > '2000-01-01';

-- select date_add with group by index having (explain)
--Testcase 4415:
EXPLAIN VERBOSE
SELECT c2, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c3 FROM time_tbl group by 3, 2, 1 HAVING date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) > '2000-01-01';
...
--Testcase 4417:
EXPLAIN VERBOSE
SELECT c2, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c3 FROM time_tbl group by 1, 2, 3 HAVING date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) > '2000-01-01';

-- select date_add with group by index having (result)
--Testcase 4418:
SELECT c2, date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval), c3 FROM time_tbl group by 1, 2, 3 HAVING date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval) > '2000-01-01';

-- select date_add and ALIAS (explain)
--Testcase 4419:
EXPLAIN VERBOSE
SELECT date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval) as date_add1 FROM time_tbl;

-- select date_add and ALIAS (result)
--Testcase 4420:
SELECT date_add(date_subtract(c3, '1 12:59:10'),  '1-2'::interval) as date_add1 FROM time_tbl;

-- select date_add with 3 arguments (explain)
--Testcase 4421:
EXPLAIN VERBOSE
SELECT date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval, 'Europe/Warsaw') FROM time_tbl;

-- select date_add with 3 arguments (result)
--Testcase 4422:
SELECT date_add((c2 + '1 d'::interval)::timestamp with time zone, '1-2'::interval, 'Europe/Warsaw') FROM time_tbl;
...
-- test json_value() of PostgreSQL
-- select json_value with normal format
-- PostgreSQL 15.0 reverts SQL/JSON features
-- --Testcase 4389:
-- EXPLAIN VERBOSE
-- SELECT json_value(c1::jsonb, '$.a' RETURNING int), json_value(c1::jsonb, '$[1]' RETURNING int) FROM s8;
-- --Testcase 4390:
-- SELECT json_value(c1::jsonb, '$.a' RETURNING int), json_value(c1::jsonb, '$[1]' RETURNING int) FROM s8;

-- --Testcase 4391:
-- EXPLAIN VERBOSE
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE to_hex(id) = '2';
-- --Testcase 4392:
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE to_hex(id) = '2';

-- --Testcase 4393:
-- EXPLAIN VERBOSE
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE id != 0;
-- --Testcase 4394:
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE id != 0;

-- --Testcase 4395:
-- EXPLAIN VERBOSE
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE json_value(c1::jsonb, '$.a' DEFAULT 0 ON EMPTY)::int > 1;
-- --Testcase 4396:
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE json_value(c1::jsonb, '$.a' DEFAULT 0 ON EMPTY)::int > 1;

-- --Testcase 4397:
-- EXPLAIN VERBOSE
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::decimal = 49.95;
-- --Testcase 4398:
-- SELECT json_value(c1::jsonb, '$.a'), json_value(c1::jsonb, '$[1]'), json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::numeric FROM s8 WHERE json_value('{"item": "shoes", "price": "49.95"}'::jsonb, '$.price' RETURNING DECIMAL(10, 1))::decimal = 49.95;

-- -- Cast wrong type, should error. 
-- --Testcase 4399:
-- EXPLAIN VERBOSE
-- SELECT json_value(c1::jsonb, '$.a' RETURNING date ERROR ON ERROR)::date FROM s8;
-- --Testcase 4400:
-- SELECT json_value(c1::jsonb, '$.a' RETURNING date ERROR ON ERROR)::date FROM s8;

-- test json_value() of MYSQL
