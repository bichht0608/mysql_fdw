...
-- Test any_value
CREATE FOREIGN TABLE agg_anyvalue(id int OPTIONS (key 'true'), c1 json, c2 int) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_core', table_name 'agg_anyvalue');

-- Test any_value any_value with integer value
DELETE FROM agg_anyvalue;
INSERT INTO agg_anyvalue(id, c2) VALUES (1, 1), (2, 2), (3, 3);
EXPLAIN VERBOSE SELECT any_value(c2) FROM agg_anyvalue;
SELECT any_value(c2) FROM agg_anyvalue;

-- Test any_value with NULL value
DELETE FROM agg_anyvalue;
INSERT INTO agg_anyvalue(id, c2) VALUES (1, NULL);
EXPLAIN VERBOSE SELECT any_value(c2) FROM agg_anyvalue;
SELECT any_value(c2) FROM agg_anyvalue;

-- Test any_value with mix of NULL value and integer values.
DELETE FROM agg_anyvalue;
INSERT INTO agg_anyvalue(id, c2) VALUES (1, NULL), (2, 1), (3, 2);
EXPLAIN VERBOSE SELECT any_value(c2) FROM agg_anyvalue;
SELECT any_value(c2) FROM agg_anyvalue;

-- Test any_value with array (MySQL does not support any array type, use JSON instead)
DELETE FROM agg_anyvalue;
INSERT INTO agg_anyvalue(id, c1) VALUES (1, '["hello", "world"]');
EXPLAIN VERBOSE SELECT any_value(c1) FROM agg_anyvalue;
SELECT any_value(c1) FROM agg_anyvalue;
...
-- From version 8.0.31, MySQL had changed the way the function variance() is calculated.
-- This test case is no longer supported for too big numbers as infinity.
...
--Testcase 240:
create foreign table t1(f1 int, f2 int) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_core', table_name 't1');
--Testcase 241:
create foreign table t2(f1 bigint, f2 oid) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_core', table_name 't2');
...
explain verbose select f2, count(*) from
...
create foreign table pagg_test (x int, y int, id int options (key 'true')) SERVER mysql_svr OPTIONS (dbname 'mysql_fdw_core', table_name 'pagg_test');
insert into pagg_test(x, y)
select (case x % 4 when 1 then null else x end), x % 10 as y
...
delete from agg_anyvalue;
insert into agg_anyvalue(id, c2) values (1, 1), (2, 2), (3, 3);
explain verbose select any_value(c2) filter (where c2 > 2) from agg_anyvalue;
select any_value(c2) filter (where c2 > 2) from agg_anyvalue;
